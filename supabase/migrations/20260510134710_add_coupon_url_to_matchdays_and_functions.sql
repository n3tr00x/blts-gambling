drop function if exists "public"."add_round"(p_round_type_id integer, p_round_date date, p_is_hit boolean, p_picks jsonb, p_votes jsonb, p_related_matchday_id integer);

drop function if exists "public"."update_round"(p_matchday_id integer, p_round_type_id integer, p_round_date date, p_is_hit boolean, p_picks jsonb, p_votes jsonb, p_related_matchday_id integer);

drop view if exists "public"."all_matchdays_for_selection";

drop function if exists "public"."get_round"(p_matchday_id integer);
...
CREATE OR REPLACE FUNCTION public.add_round(p_round_type_id integer, p_round_date date, p_is_hit boolean, p_picks jsonb, p_votes jsonb, p_coupon_url text DEFAULT NULL::text, p_related_matchday_id integer DEFAULT NULL::integer)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.update_round(p_matchday_id integer, p_round_type_id integer, p_round_date date, p_is_hit boolean, p_picks jsonb, p_votes jsonb, p_related_matchday_id integer DEFAULT NULL::integer, p_coupon_url text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_round(p_matchday_id integer)
 RETURNS TABLE(round_number integer, round_type text, round_date date, is_hit boolean, season text, related_matchday_id integer, coupon_url text, picks jsonb, votes jsonb)
 LANGUAGE sql
 SECURITY DEFINER
AS $function$
SELECT
...
CREATE OR REPLACE FUNCTION public.get_round_for_edit(p_matchday_id integer)
 RETURNS TABLE(round_type_id integer, round_date date, round_number integer, is_hit boolean, related_matchday_id integer, coupon_url text, picks jsonb, votes jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
